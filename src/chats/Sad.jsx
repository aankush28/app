...
import { List, Image, Button, Icon ,Popup,Label, Card,Message, Segment,Header, Input} from 'semantic-ui-react'
  ...
  <Card>
      <Card.Content>
        <Image
          floated='right'
          size='mini'
          src='https://i.pinimg.com/564x/90/44/db/9044db587ad902560de801ba7c62f4b4.jpg'
        />
        <Card.Header>Face look</Card.Header>
        <Card.Meta>Friends of sad</Card.Meta>
        <Card.Description>
           add your to the group <strong>best friends</strong>
        </Card.Description>
      </Card.Content>
      <Card.Content extra>
    ...
    <Message size='mini' 
  compact color='teal'
  content={items.map((itemval) => {
    return <p><Icon name='trash alternate' />{itemval}</p>
    })}/>
<hr/> 
    <p>{inputList}</p>
    ...
    <Header as='h3' inverted color='teal' block  className="Maniesh_massage" >
  <Input labelPosition='right' type='text' placeholder='type a text message..............' size='large' onChange={itemEvent}  value={inputList} >
    <Label icon='smile outline' ></Label>
    <input />
    <Label icon="send" onClick={listOfItems}></Label>
  </Input>
   </Header>
</div>
        </div>
